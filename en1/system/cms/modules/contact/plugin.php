	...
	public function form()
	{
		...
		if ($this->input->post('contact-submit')) { 
			...
			if ($this->form_validation->run())
			{
				...
				# Custom Check for Recapcha by Google (Thanks Google!) and Oscar L. <anduryn@gmail.com>... #
				$fields = array(
					'secret'			=> '6LdI2SAUAAAAAFIlABnfKBzrJBoAkaSaBwXmzvOC',
				...
				if(!$array->success || empty($array->success))
				{
					$this->session->set_flashdata('error', 'Please complete the anti-spam barrier.');
				...
				$this->session->set_flashdata('contact-sent', $message);
